...
def ingest(
    ...
    if skip_duplicates:
        ...
        # rstrip("/") so that the filesystem root ("/") produces the prefix
        # "/%" instead of "//%" — both are technically valid LIKE patterns
        # but only the former matches children of root like "/photos/...".
