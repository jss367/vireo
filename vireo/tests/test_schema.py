...
def test_failed_registry_migration_rolls_back_version_and_data(tmp_path, monkeypatch):
    ...
    migration = schema.Migration(7, "interrupted", fail_after_write)
...
def test_navigation_migration_changes_only_untouched_legacy_default(tmp_path):
    import json
    ...
    with Database(db_path) as db:
        ...
        db.conn.execute(
            ...
            (json.dumps(schema._LEGACY_DEFAULT_TABS), workspace_id),
        )
    ...
    assert json.loads(rows["Default"]) == schema._PRIMARY_WORKFLOW_TABS
