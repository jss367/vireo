...
def test_merge_keyword_rewrites_pending_changes_to_dst_name(db):
    ...
    Pending rows are seeded with raw INSERTs: ``queue_change`` now
    normalizes keyword values at write time, so legacy spellings can only
    exist as pre-normalization rows — exactly what the merge rewrite is
    there to clean up."""
    ws = db.create_workspace("A")
    ...
    # Seed an unsynced keyword_add under the legacy spelling. Simulates a
    # tag applied before this normalization pass landed.
    # Also seed a duplicate under the destination name so we exercise the
    # dedupe branch (the merge must drop the source row without producing
    # two identical pending entries).
    for token, pid, value in (
        ("tok-a-legacy", pid_a, "‘apapane"),
        ("tok-b-clean", pid_b, "apapane"),
        ("tok-b-legacy", pid_b, "‘apapane"),
    ):
        db.conn.execute(
...
def test_merge_duplicate_keywords_scopes_curation_to_tagged_pairs(db):
    ...
    Pending rows are seeded with raw INSERTs: ``queue_change`` now
    normalizes keyword values at write time, so legacy spellings only
    exist as pre-normalization rows.
    """
    ...
    for token, pid, ws in (
        ("tok-a-legacy", pid_a, ws_a),
        ("tok-b-legacy", pid_b, ws_b),
    ):
        db.conn.execute(
            """INSERT INTO pending_changes
                  (photo_id, change_type, value, change_token, workspace_id)
               VALUES (?, 'keyword_add', ?, ?, ?)""",
            (pid, "‘apapane", token, ws),
        )
    db.conn.commit()
