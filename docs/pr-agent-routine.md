...
- `fix-main` — Full tests failed on `main` (fired by `main-health.yml` until one request per `main-red` incident has been accepted); the routine opens a `fix-main` PR. The dispatch is gated on the repository variable `MAIN_HEALTH_ENABLE_FIX_MAIN`, which the maintainer flips to `true` only after pasting the updated `pr-agent-routine-prompt.md` into the routine at claude.ai/code/routines. Until then `main-health.yml` still tracks failing runs in the `main-red` issue but never calls the routine.
