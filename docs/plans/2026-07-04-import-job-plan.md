...
- Assert: every discovered file exists under the archive at its template path; a photo row exists **for each copied file at its final path** (join folders on path); each row has `file_hash` set, `hash_status = 'ok'`, `hash_checked_at` non-NULL; the result dict reports `{discovered, copied, verified, skipped_duplicate, failed}` consistently; folders are linked to the active workspace.
...
- Per file in a batch: dedup `match()`. Hash matches can record a skip with their token and matched archive folder path immediately. Key-only matches must be hash-checked before they are accepted as duplicates (Task 2.3 pins the exact ledger contract): if bytes match, record the duplicate skip and destination folder; if bytes differ, fall through to the normal copy path so the card file is imported as a distinct photo. Non-duplicates call `copy_and_hash_verify` (pass the checker's cached hash when it has one). Collision handling mirrors `ingest()` (numeric suffix when same name + different content at destination; skip when the destination file is byte-identical — reuse `ingest()`'s existing check shape).
- Per batch, after all copies: `scan(root=destination, db, restrict_dirs=[batch dest dir plus any verified duplicate-match destination dirs], restrict_files=[verified dest paths], vireo_dir=…, thumb_cache_dir=…, cancel_check=…)`, collecting `(photo_id, path)` via `photo_callback`; then `UPDATE photos SET file_hash=?, hash_status='ok', hash_checked_at=? WHERE id=?` for each freshly verified copy. This keeps duplicate-only imports from reporting success while leaving an unlinked workspace empty and uses the existing integrity-audit verdict vocabulary.
...
- A card file that key-matches (same name, size, trusted capture time) a cataloged twin **whose bytes differ** is not accepted as a duplicate: it is copied through the normal same-name/different-content collision path, cataloged as a distinct photo, and the result stays `safe_to_format is True` when that copy verifies. Build the fixture from `test_import_dedup.py`'s `_write_jpeg(..., exif_dt=...)` and `_seed_photo(...)` pattern: seed the catalog row with the same trustworthy `photos.timestamp` (for example `2026-05-01T10:15:30`) and create the card file with matching `DateTimeOriginal`, name, and size but different bytes. Do not rely on mtime alone; `DuplicateChecker` key matches use EXIF/catalog capture timestamps.
- A failed copy → unsafe.

**Step 2:** Run — FAIL.

**Step 3:** Implement the per-file ledger: every discovered file ends in exactly one terminal bucket — `verified_fresh`, `verified_duplicate`, or `failed`. A key-only duplicate candidate is a transient state only: hash the card file and the cataloged twin at its archive path before skipping. Equal bytes → `verified_duplicate`; different bytes → requeue that source through the normal copy/collision path and bucket it as `verified_fresh` after the copy verifies. `safe_to_format = no failures and every file is in a verified terminal bucket`. Result carries counts + `unsafe_files` (path + reason, for failed copies or unreadable verification inputs) — a pill that means exactly what it says.
...
**Step 2–4:** standard loop. Mount-path catalog rows get `hash_status='ok'` and `hash_checked_at` only on the checksum verification path; otherwise leave `hash_status`/`hash_checked_at` NULL rather than inventing new status values. The safe-to-format pill requires local `ok` rows or hash-backed duplicates, so remote imports without checksum verification honestly report `safe_to_format False` with reason "enable verify_by_hash for remote verification".
