...
- `vireo/repositories/` — Per-domain SQL behind the `Database` façade: `workspaces.WorkspaceRepository` (workspace rows, config-override rewrites, label-set selection, navigation tabs, new-images snapshots), `photo_labels.PhotoLabelRepository` (color labels) and `photo_review.PhotoReviewRepository` (ratings, flags); `audit.AuditRepository` (audit-run records, hash-integrity queries and verdicts). `Database` keeps every public method as a thin wrapper that builds the repository on `self.conn` (`_workspace_repository(scoped=...)`, `_photo_label_repository()`, ...) and passes the active workspace id explicitly; the active-workspace state, `_ws_id()` and the new-images cache stay on `Database`. `repositories.UNSET` is the shared "argument not provided" sentinel (`db._UNSET`). Structural tests (`test_workspace_method_delegates_to_repository` in `vireo/tests/test_db_workspaces.py`) fail if a moved method touches `self.conn` again. `edits.EditsRepository` (per-photo edit recipes and global edit presets; not workspace-scoped, built by `_edits_repository()`; the optional workspace check stays in the wrapper; structural test in `vireo/tests/test_db_edits.py`). Also `caches.CachesRepository` (preview-cache LRU, offline originals; catalog-wide, and `_caches_repository()` passes in `db.execute_with_retry` / `db.commit_with_retry`). `processes.ProcessesRepository` (saved processes; global, so `_processes_repository()` takes no workspace). `model_runs.ModelRunsRepository`: detector and classifier run records, match scores, the classify preflight's cache-hit and unclassifiable sets, and labels fingerprints (`_model_runs_repository()`; structural test in `vireo/tests/test_db_model_runs.py`). `inat.InatRepository` holds the catalog-wide iNaturalist submissions (`_inat_repository()`). `duplicates.DuplicatesRepository` holds the exact-duplicate group SQL (catalog-wide, no workspace id); the pure resolver it feeds is the top-level `vireo/duplicates.py`, and the merge's `tag_photo` calls and provenance fold stay on `Database._apply_winner_loser_merge`. `masks_features.MasksFeaturesRepository` holds SAM mask rows and variant activation, the mask-cleanup deletes, pipeline-feature and raw-analysis writes, the mask and eye-keypoint stage selectors, and embeddings (`_masks_features_repository(scoped=...)`); the detector floor, `_scope_clause` and the masks-directory containment check (`_safe_remove_mask_file`, passed to the deletes as a callback) stay on `Database`. `sync.SyncRepository` holds the pending XMP queue SQL (`pending_changes`); `_sync_repository()` passes `Database._ws_id` as a resolver so the active workspace is still read lazily, and the composition (`remove_pending_changes` queueing an inverse through `queue_change`, flat-removal equivalence, flag-sync config) stays on `Database` (structural test in `vireo/tests/test_db_sync.py`). `moves_merge.MovesMergeRepository` holds move rules, `move_folder_path` / `batch_update_photo_folder`, the move-rule match query, sync-only grants and the staged-tree → archive merge (`_moves_merge_repository()`, catalog-wide); the merge receives the façade methods it composes (workspace-folder links, the `_*_for_merge` helpers, `update_folder_counts`) as callbacks, and the keyword re-tags stay on `Database` (`_move_location_state_for_merge`, `_carry_keyword_associations_for_merge`) so `test_keyword_provenance_contract` still sees them; structural tests in `vireo/tests/test_db_moves_merge.py`.
