...
- `vireo/web/media.py` — Image and mask serving: `/thumbnails/<filename>`, every `/photos/<id>/*` render (`crop`, `full`, `preview`, `edit-mask-preview`, `edit-preview`, `original`), `/masks/<filename>` and `/api/masks/<pid>/<variant>.png`, and the per-photo variant listing `/api/photos/<pid>/masks` that points at them, each gated to the active workspace. The helpers only these routes use live here: `_requested_pair_source` (`?source=jpeg|raw`) and the paired shadow-render paths, `_ArtifactResponseError`, `_shed_prefetch_response`, the full-resolution render cache (`_full_resolution_render_path`, `_prepared_full_resolution_render`, mtime pegging), `_serve_trusted_working_copy`, and the mask readers. Built by `create_media_blueprint(get_db, json_error, db_path, config, ...)`; the shared preview-invalidation state (`invalid_preview_cache_paths`, `clear_preview_cache_invalid`) is injected from `create_app`, whose edit-recipe cache invalidation writes it, and so is the app's `_photo_not_found_error` (optional; it defaults to an equivalent response). `create_app` binds `serve_original_photo` to the registered `media.serve_original_photo` view so the prepare-full-resolution job renders through it. Tests that patch `working_copy_publication_guard`, `touch_working_copy_access` or `_recipe_render_source` for these routes patch `web.media`.
...
- `vireo/web/jobs.py` — Job-control routes (`/api/jobs`, status, cancel, pause, resume, stream, history) and the self-contained launchers (thumbnails, duplicate scan, verify-hashes, model/taxonomy downloads, capture-time, sharpness, cull, regroup). Built by `create_jobs_blueprint(...)`; launchers that need helpers shared with `app.py` live in `web/job_launchers.py`.
- `vireo/web/job_launchers.py` — The remaining `/api/jobs/*` launchers: scan, scan-workspace, repair-metadata, previews, ingest, move-photos, move-folder, offline-cache, prepare-full-resolution, sync, classify, develop, extract-masks, fetch-labels, precompute-embeddings, batch-delete. Built by `create_job_launchers_blueprint(...)`; the helpers they share with routes and startup work still in `create_app` (`_build_scan_work`, `_run_batch_delete`, missing-originals invalidation, the move-folder guard/starter, `_metadata_repair_count`, the settings lock and raw config reader, the `/photos/<id>/original` view, `app._sync_job_lock`, `_chunked`) are injected.
...
- `vireo/web/workspaces.py` — Workspaces domain: every `/api/workspaces/*` endpoint owned by the core app (CRUD, activate, pin, folders GET/POST/DELETE, move-folders, active config GET/POST, subject types GET/PUT, new-images probe/recheck/snapshot create/get) plus the `/api/workspace/tabs/*` navigation routes and `/api/workspace/classification-inventory` (the dashboard's model × label-set coverage panel, with its module-level `_inventory_pair` / `_inventory_subtotal` row builders). `_validate_workspace_config_overrides` and the `_new_images_walk_*` / `_new_images_deferred_reason` helpers are closures in the factory; `_NEW_IMAGES_SYNC_WAIT_SECS` is a module constant. The `/api/workspaces/active/...` local-workspace and local-folder staging routes have their own blueprints (`web.local_workspace` / `web.local_folder`). Built by `create_workspace_blueprint(...)`; `get_runner`, `invalidate_missing_originals`, `settings_write_lock`, `new_images_walk_progress` (the `app._new_images_walk_progress` dict) and `missing_originals_heavy_job_types` are injected from `create_app`.
- `vireo/web/folders.py` — Folders domain: every `/api/folders*` endpoint (tree with optional `with_workspace` snapshot, missing, check-health, detail, per-folder workspaces, relocate, delete, rescan job, bulk reveal in the OS file manager). Built by `create_folders_blueprint(get_db, json_error, get_runner, db_path, ...)`; the rescan job builds its own `background_job` via `make_background_job`. `build_scan_work` (shared with the `/api/jobs/scan*` launchers), `cleanup_cached_files_for_deleted_photos` and `invalidate_missing_originals` are injected from `create_app`; workspace membership of folders stays in `web/workspaces.py`.
- `vireo/web/capture_time.py` — `/api/capture-time/preview`, the capture-time correction preview shown before the `/api/jobs/capture-time` job (in `web/jobs.py`) runs; the correction math lives in the top-level `capture_time` module. Built by `create_capture_time_blueprint(get_db, json_error)`.
- `vireo/web/remote_setup.py` — NAS setup wizard: every `/api/remote-setup/*` endpoint (network mounts, ssh-check, install-key, locate-share, list-remote-dirs, disk-free, check-archive-root), all loopback-only. Built by `create_remote_setup_blueprint(...)`; the loopback guard, connection-argument validation and ssh-binary lookup live with the routes, so nothing beyond `get_db` and `json_error` is injected.
...
- `vireo/web/history.py` — Edit history: `/api/undo`, `/api/redo` (each with a `/status` probe) and `/api/edit-history`, with the follow-ups only undo/redo use (`_edit_recipe_history_updates`, `_refresh_pipeline_cache_species_from_edit`, `_history_flags_changed`). Undo and redo are prediction-decision routes and run under `services.prediction_decisions`. Built by `create_history_blueprint(get_db, json_error, db_path, ...)`; `_invalidate_photo_render_cache` and `_queue_edit_recipe_sync` are injected from `create_app` because the edit-recipe routes share them. Per-photo edit-recipe history (`/api/photos/<id>/edit-history`) is in `web/photo_edit_recipes.py`.
- `vireo/web/photos.py`, `vireo/web/photo_edit_recipes.py`, `vireo/web/photo_location_keywords.py` — The `/api/photos` data API, split by concern. `web.photos`: listing and paging (`/api/photos`, `query`, `ids`, `by-ids`, `calendar`, `geo`, `companion-count`), photo detail (`/api/photos/<id>`, aliased as `/api/v1/photos[/<id>]` — the alias table names `photos.api_photos` / `photos.api_photo_detail`, so this blueprint is registered before the alias loop), text and similarity search, best-batch, region sharpness, subjects and primary subject, the wildlife-exclusion toggle, open-external, supported extensions, and the Missing Originals routes (`/api/photos/missing*`); built by `create_photos_blueprint(...)` with `MAP_RENDER_PHOTO_LIMIT` as a module constant, and the app's `visual_scope`, `_photo_not_found_error`, `_MAX_PER_PAGE`, the flag/location-status request parsers, the Missing Originals cache helpers, `_run_batch_delete`, and the highlight/best-batch helpers injected from `create_app`. `web.photo_edit_recipes`: per-photo edit-recipe get/set/clear/compose, the local-mask snapshot, bulk apply and summary, and edit-recipe history, with the local-mask helpers as module functions; `_invalidate_photo_render_cache` and `_queue_edit_recipe_sync` are injected. `web.photo_location_keywords`: per-photo keyword add/remove and location set/set-from-text/clear, using `web.location_edits` and `services.pending_changes`; the app's `location_errors` is injected. `/api/photos/<id>/masks` is in `web/media.py` with the mask routes and `/api/photos/<id>/pipeline` is in `web/pipeline.py`.
...
- `vireo/web/batch.py` — Batch edits over a photo selection: every `/api/batch/*` endpoint (`location/text`, `location`, `location/from-exif`, `best-batch-flags`, `keyword`, `keyword-remove`, `wildlife-excluded`, `delete`), using `web.location_edits`, `web.request_args.parse_selection_photo_ids`, `services.pending_changes` and `places`. Built by `create_batch_blueprint(...)`; the app's `location_errors`, `_bulk_gps_location_payload` (shared with the imports blueprint), `_run_batch_delete` (shared with the photos and job-launchers blueprints) and missing-originals invalidation are injected from `create_app`.
- `vireo/web/locations.py` — Places and location review: `/api/places/reverse-geocode`, `/api/keywords/<id>/link-place` (a place operation, so it lives here rather than in `web/keywords.py`), and every `/api/location-review/*` endpoint (preview, resolve-discrepancies, saved-suggestions), with the `_location_review_groups` coordinate clustering and `_serialize_keyword` payload only these routes use. Built by `create_locations_blueprint(...)`; `location_errors` and the helpers shared with the EXIF-GPS payload builder `_bulk_gps_location_payload` still in `app.py` (photo-id parsing, GPS source ids, location-keyword lookup, the Google reverse-geocode call, the reverse-geocode cache codec, and the place summary) are injected from `create_app`.
...
- `vireo/services/prediction_decisions.py` — The prediction-decision writer lock shared by every route that records a review decision (batch/single accept and reject, reviewed, subject species, replace keywords, group apply, highlights confirm/relabel, undo/redo; encounters takes it too): `begin_prediction_decision(db, json_error=...)`, `under_prediction_decision_lock(db, work, json_error=...)`, `out_of_workspace_prediction_ids(db, pred_ids)`, and `PREDICTION_DECISION_ROUTES` (view function names, not endpoint names, so an entry survives a move into a blueprint). Routes in `app.py` or `vireo/web/` call these through the module (`from services import prediction_decisions`), never under an injected alias. `test_every_prediction_decision_route_locks` derives the decision routes from the call graph of `app.py`, `vireo/web/*.py` and `vireo/services/*.py` and fails until a new one is listed and reaches `begin_prediction_decision`.
