...
Layering: `vireo/app.py` wires the app; HTTP routes live in blueprints under `vireo/web/`; workflow, filesystem and cache logic lives in `vireo/services/` (which never imports from `vireo/web/`); SQL lives in `vireo/repositories/` (and the canonical schema in `vireo/canonical_schema.py`), behind the `Database` façade in `vireo/db.py`. See `docs/ARCHITECTURE.md` for the boundaries.
...
- `vireo/repositories/` — Per-domain SQL behind the `Database` façade, one module per domain (`<domain>.<Domain>Repository`); each module's docstring says what it owns and what deliberately stays on `Database`. Conventions: `Database` keeps every public method as a one-line wrapper that builds the repository on `self.conn` via `_<domain>_repository(...)`, passing the active workspace id (or `self._ws_id` as a callable where resolution is lazy), `commit_with_retry`, and module constants in explicitly; composition across domains, process caches and the active-workspace state stay on `Database`; every `photo_keywords` writer that creates or converges an association lives in `repositories/keyword_provenance.py`, and other domains reach it through the façade (`self.tag_photo(...)`) so `test_keyword_provenance_contract` sees each call site; `repositories.UNSET` is the shared "argument not provided" sentinel (`db._UNSET`). Each domain's `vireo/tests/test_db_<domain>.py` has a structural test that fails if a moved method touches `self.conn` again, and `vireo/tests/test_db_facade_structure.py` holds the whole class to it: outside `__init__`, `close` and `_commits_held`, a `Database` method may only hand `self.conn` to a repository. New domains document themselves in their module docstring, not on this line.
