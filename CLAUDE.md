...
6. When **Full tests fails on `main`**, `main-health.yml` opens (or comments on) a `main-red` issue and — when the repository variable `MAIN_HEALTH_ENABLE_FIX_MAIN` is `true` — fires the routine with `Task: fix-main`, which opens a `fix-main` PR. It won't fire while a `fix-main` PR is open, after three attempts on one issue, or while an earlier attempt is still in flight (its reservation marker is posted but no matching PR yet); the issue closes on the next green run. The gate variable is off by default because the stored routine prompt is not synced from the repo — flip it to `true` after pasting `pr-agent-routine-prompt.md` into the routine. Runs on `main` are never cancelled mid-run (a burst of merges queues one run for the newest), so main always gets a complete result.
